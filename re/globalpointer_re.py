...
def evaluate(data):
    ...
    f = open('dev_pred_globakpointer.json', 'w', encoding='utf-8')
    pbar = tqdm()
    for d in data:
        R = set([SPO(spo) for spo in extract_spoes(d['text'])])
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if f1 >= self.best_val_f1:
            self.best_val_f1 = f1
            model.save_weights('../checkpoint/re/best_model_globalpointer.weights')
...
if __name__ == '__main__':
    ...
    if do_predict:
      model.load_weights('../checkpoint/re/best_model_globalpointer.weights')
      predict(valid_data)